...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        logging.info('found model: %s (table: %s)' % (name, table_name))  # 日志：找到名为 name 的 model
        # 获取 所有的 Field 和主键名
        mappings = dict()
        fields = []
        primary_key = None
        ...
        for k, v in attrs.items():
            ...
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    ...
                    # 找到主键，如果主键 primary_key 有值时，返回一个错误
        ...
        attrs['__table__'] = table_name  # table 名

        attrs['__primary_key__'] = primary_key  # 主键属性名

        attrs['__fields__'] = fields  # 除主键外的属性名

        # 构造默认的 SELECT, INSERT, update和 DELETE 语句
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def getValueOrDefault(self, key):
        ...
        if value is None:  # 如果 value 为 None，定位某个键； value 不为 None 就直接返回
    ...
    async def findAll(cls, where=None, args=None, **kw):
        ...
        if limit is not None:
            sql.append('limit')
            if isinstance(limit, int):                # 如果 limit 为整数
                sql.append('?')
                args.append(limit)
        ...
        rs = await select(' '.join(sql), args)        # 返回选择的列表里的所有值 ，完成 findAll 函数
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        ...
        if len(rs) == 0:  # 如果 rs 内无元素，返回 None ；有元素就返回某个数
            return None
        return rs[0]['_num_']
